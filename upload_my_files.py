...
# настройка логгирования, logging.DEBUG (уровень выводимых ошибок) позволяет видеть сообщения отладки и информационные
logging.basicConfig(format=u'%(filename)s [ LINE:%(lineno)+3s ]#%(levelname)+8s [%(asctime)s]  %(message)s',
                    level=logging.DEBUG)

# соединение с БД, создаем движок SqlAlchemy
engine = create_engine(f'sqlite:///{DB_FILENAME}')

# os.path.isfile - проверяет является ли указанный путь файлом, если нет, то команда Base.metadata.create_all создает таблицу
if not os.path.isfile(f'./{DB_FILENAME}'):
    Base.metadata.create_all(engine)

# создаем класс сессии, bind - привязывает сессию бд к определенному движку, который применяется для установки подключения
session_factory = sessionmaker(bind=engine)
# scoped_session – это хранилище уже созданных сессий, каждая из которых привязана к своему треду
...
async def uploadMediaFiles(folder, method, file_attr):
    folder_path = os.path.join(BASE_MEDIA_PATH, folder)     # os.path.join - соединяет пути с учётом особенностей операционной системы.
    for filename in os.listdir(folder_path):       # os.listdir - список файлов и директорий в папке
