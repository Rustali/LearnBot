...
# бот приветствует о отправляет нам шаблон кнопки
...
async def process_hi6_command(message: types.Message):
    await message.reply('Шестое - запрашиваем контакт и геолокацию\nЭти две '
                        'кнопки не зависят друг от друга', reply_markup=kb.markup_request)
...
async def process_rm_command(message: types.Message):
    await message.reply('Убираем шаблоны соощений', reply_markup=kb.ReplyKeyboardRemove())
...
async def process_2_command(message: types.Message):
